...
sem_t tenedores_libres_sem;
sem_t tenedores_ocupados_sem;
...
{  
    sem_wait(&tenedores_libres_sem);
    pthread_mutex_lock(der(i));
    pthread_mutex_lock(izq(i));
    while(sem_trywait(&tenedores_ocupados_sem)>0){
        pthread_mutex_unlock(der(i));
        sleep(3);
    }
    sem_post(&tenedores_ocupados_sem);
}

void dejar_tenedores(int i)
{   sem_wait(&tenedores_ocupados_sem);
    pthread_mutex_unlock(der(i));
    pthread_mutex_unlock(izq(i));
    sem_post(&tenedores_libres_sem);
}
...
int main()
{
    ...
    sem_init(&tenedores_libres_sem,0,N_FILOSOFOS); //para threads:no compartido en dif proceos.
    sem_init(&tenedores_ocupados_sem,0,N_FILOSOFOS); 
    ...
    sem_destroy(&tenedores_libres_sem);
    sem_destroy(&tenedores_ocupados_sem);
    return 0;
}
...
// Funciona porque va alternando entre zurdo y diestro por lo tanto ninguno se queda esperando indefinidamente que otro le pase el tenedor, ya que espera el opuesto y no ese.
