...
int handle_cli_ev(struct eventloop_data *evd, struct epoll_event *ev, struct cinfo *ci) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        free(ci);
        return 0;
    } else if (ev->events & (EPOLLRDHUP | EPOLLHUP)) {  // cli cero conex
        log(2, "remote hangup para fd %i", ci->fd);
        rc = epoll_ctl(evd->epfd, EPOLL_CTL_DEL, ci->fd, NULL);
        if (rc < 0)
            log(1, "epoll_delete remote hangup??");  // log:login(==printf)
...
int handle_ev(struct eventloop_data *evd, struct epoll_event *ev) {
    ...
    if (ci->typ == LSOCK) {
        ...
        /* Rearmar lsock fd */  // lo pone de nuevo disponible (por EPOLLONESHOT)
...
#define NEV 16
    ...
    wait_again:
        nev = epoll_wait(epfd, ev, NEV, -1);  // si error->vuelvo a esperar
        if (nev < 0) {
            if (errno == EINTR) {
...
void server(int text_sock) {
    ...
    {
        struct cinfo *text_ci = malloc(sizeof *text_ci);  // maneja cundo el cliente esta conecado
        struct epoll_event ev;

        if (!text_ci)
            quit("oom server malloc");

        text_ci->typ = LSOCK;  // socket d escuch
	...
	event_loop(evdata_i);
#else
    int nproc = defopts.nthread == 0 ? sysconf(_SC_NPROCESSORS_ONLN) : defopts.nthread;  // tntos hilos como cpu
...
int main(int argc, char **argv) {
    ...
    kv_init(defopts.tablesz);  // inicializa la estructura

    server(text_sock);  // inicializa sr
