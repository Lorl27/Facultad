...
int main() {
  ...
  // Iniciar tabla hash
  TablaHash tabla = tablahash_crear(CAPACIDAD_INICIAL ,(FuncionHash)contacto_heashear);
  ...
  for(int i = 0; i < 6; ++i){
    ...
    int nElems = tablahash_nelementos(tabla);

    tablahash_insertar(tabla, agenda[i],contacto_destruir,contacto_copia,contacto_comparar);

    if(tablahash_nelementos(tabla)==nElems) printf("\tInsercion fallida: Colision.\n");
    else printf("\tInsercion exitosa.\n");
  }
  ...
  for(int i = 0; i < 6; ++i){
    ...
    Contacto *ret = tablahash_buscar(tabla, agenda[i],contacto_comparar);
    if(ret != NULL) printf("\tSe encuentra en la tabla.\n");
  ...
  for(int i = 5; i > 2; --i){
    ...
    tablahash_eliminar(tabla, agenda[i],contacto_comparar,contacto_destruir);
  }
  ...
  tablahash_insertar(tabla, nuevoContacto,contacto_destruir,contacto_copia,contacto_comparar);
  // Chequeamos que se haya sobrescrito
  Contacto *ret = tablahash_buscar(tabla, agenda[0],contacto_comparar); // Es equivalente a buscar nuevoContacto porque se compara por nombre
  ...
  // Liberar memoria
  tablahash_destruir(tabla,contacto_destruir);
